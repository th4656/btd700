...
static PAIRED_CACHE: Mutex<Option<(Instant, Vec<PairedDevice>)>> = Mutex::new(None);

pub fn find_paired_headsets() -> Vec<PairedDevice> {
    if let Ok(guard) = PAIRED_CACHE.lock() {
    ...
    if let Ok(output) = Command::new("bluetoothctl").args(["--timeout", "1", "devices"]).output() {
    ...
    if let Ok(mut guard) = PAIRED_CACHE.lock() {
